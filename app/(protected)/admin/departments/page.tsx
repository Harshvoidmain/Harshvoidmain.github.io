...
export default function DepartmentsPage() {
  ...
  const [newDept, setNewDept] = useState({ name: "", code: "", shortName: "" });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success(`Department [${nextId}] created.`);
      setShowAdd(false);
      setNewDept({ name: "", code: "", shortName: "" });
    } catch {
        ...
        <DialogContent>
          ...
          <div className="px-6 py-4 space-y-4">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800 border border-border">
              <span className="text-xs text-muted">Auto-assigned ID:</span>
              <DeptIdBadge deptId={nextId} />
            </div>
            <Input
            ...
            {newDept.name && newDept.code && (
              <div className="p-3 rounded-lg border border-border text-sm text-muted">
                Preview: <span className="dept-id-badge">[{nextId}]</span>{" "}
                <span className="font-medium text-[rgb(var(--text-primary))]">{newDept.code}</span> — {newDept.name}
              </div>
            )}
          </div>
          <DialogFooter>
