...
export default function UsersPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setUsers(snap.docs.map((d) => ({ uid: d.id, ...d.data() }) as UserDocument).filter(u => !u.blocked));
      setLoading(false);
    });
    return () => unsub();
  }, []);
  ...
  const handleDelete = async () => {
    ...
    try {
      await updateDocument(`users/${deleteUser.uid}`, { blocked: true, updatedAt: serverTimestamp() });
      await writeAuditLog({
